...
export function TimelineList({ items }: { items: TimelineItem[] }) {
  ...
  return (
    <dl ref={listRef} onKeyDown={onKeyDown} className="relative mx-auto mt-10 max-w-6xl lg:mt-14">
      ...
      {items.map((item, index) => {
        ...
        return (
          <div
            ...
            className={`timeline-item relative grid grid-cols-[2.5rem_1fr] gap-x-2 py-6 transition-opacity duration-500 md:grid-cols-[1fr_5rem_1fr] md:gap-x-0 md:py-10 lg:py-14 ${isActive ? "opacity-100" : "opacity-50 hover:opacity-90 focus-within:opacity-100"}`}
          >
            <dt className="col-start-2 md:col-start-1 md:row-start-1 md:pr-8 md:text-right lg:pr-12">
              ...
              >
                <span
                  aria-hidden="true"
                  className={`absolute left-5 top-9 z-10 h-3.5 w-3.5 -translate-x-1/2 rounded-full border-2 transition-all duration-300 group-hover:scale-125 md:left-1/2 md:top-[3.25rem] lg:top-[4.75rem] ${isActive ? "scale-150 border-accent bg-accent shadow-[0_0_0_6px_rgb(var(--color-accent)/0.2)]" : "border-ink/60 bg-paper"}`}
                />
                {item.year}
              </button>
            </dt>
